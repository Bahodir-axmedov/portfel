			...
			<Container>
				...
				{tiles.length > 0 ? (
					<div className="mt-14">
						...
						>
							{tiles.map((tile) => (
								<StaggerItem key={tile.id} variant={scaleIn}>
									<a
										href={tile.target}
										target="_blank"
										rel="noreferrer"
										className="group relative flex h-full flex-col items-center gap-3 overflow-hidden rounded-xl border border-line bg-glass p-3.5 transition-[transform,border-color,box-shadow] duration-500 ease-premium hover:-translate-y-1.5 hover:border-brand-500/50 hover:shadow-[0_22px_50px_-24px_rgba(59,130,246,0.9)] active:scale-[0.98]"
									>
										<span
											className="w-full overflow-hidden rounded-lg bg-white p-2 transition-transform duration-500 ease-premium group-hover:scale-[1.04] [&>svg]:block [&>svg]:h-auto [&>svg]:w-full"
											dangerouslySetInnerHTML={{ __html: tile.svg }}
										/>
										{/* Scanner line, only while hovered. */}
										<span aria-hidden className="qr-scan" />
