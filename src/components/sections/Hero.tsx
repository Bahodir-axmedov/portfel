 ...
 * is a 4:5 crop with the subject's head top-centre, so every anchor sits on the
 * left or right flank and none is placed above the face. A generated ring would
 * eventually park a chip on the subject's head, which is exactly the detail
 * that makes a layout look machine-made.
 *
 * Slots alternate left/right down the frame so the eye zig-zags instead of
 * reading two static columns, and `delay` staggers the float cycle so no two
 * chips bob in sync.
 */
const BADGE_SLOTS = [
	{ top: "7%", left: "-12%", delay: "0s" },
	{ top: "19%", right: "-11%", delay: "0.7s" },
	{ top: "33%", left: "-15%", delay: "1.4s" },
	{ top: "45%", right: "-14%", delay: "2.1s" },
	{ top: "59%", left: "-13%", delay: "2.8s" },
	{ top: "71%", right: "-10%", delay: "3.5s" },
	{ top: "85%", left: "-7%", delay: "4.2s" },
	{ top: "93%", right: "-5%", delay: "4.9s" },
] as const

/**
 * Angles, in degrees, of the light points that orbit behind the portrait.
 *
 * Six points at 60deg spacing: enough to read as a continuous orbit, few enough
 * that the ring never becomes a dotted circle. The ring renders behind the
 * frame, so only the arc that clears the photo is visible — that partial
 * occlusion is what makes the portrait feel like it sits *in* the scene rather
 * than on top of it.
 */
const ORBIT_DOTS = [0, 60, 120, 180, 240, 300] as const
...
export function Hero({
	...
	return (
		<section
			id="top"
			className="relative isolate overflow-hidden pb-[64px] pt-[104px] md:pb-[104px] md:pt-[148px]"
		>
			{/* Signature of the whole site: a slowly turning planet, cropped by the
			    section so it reads as enormous. Pure CSS — see ui/Planet.tsx. */}
			<Planet className="hero-planet" />
				...
				<div className="grid items-center gap-14 lg:grid-cols-[minmax(0,1.08fr)_minmax(0,0.92fr)] lg:gap-16">
					...
					>
						...
						>
							<TypingText
								words={roles}
								options={{ typeSpeed: 58, deleteSpeed: 26, holdTime: 1800 }}
							/>
						</motion.div>
					...
					>
						{/* Orbiting light points, behind the portrait. */}
						<div aria-hidden className="orbit-field -z-10 hidden md:block">
							...
							<div className="orbit-spin">
								{ORBIT_DOTS.map((angle) => (
									<span
										...
										style={{
											transform: `translate(-50%, -50%) rotate(${angle}deg) translateY(-232px)`,
										}}
									>
										<span className="block h-1.5 w-1.5 rounded-full bg-brand-400 shadow-[0_0_12px_rgba(96,165,250,0.85)]" />
									</span>
								))}
							</div>
						</div>
						...
						<div className="photo-frame will-animate" style={photoShift}>
							<div className="relative overflow-hidden rounded-[26px] border border-white/[0.08] bg-base/70 p-2.5 backdrop-blur-xl">
								...
								<div className="relative aspect-[4/5] w-full overflow-hidden rounded-[22px]">
									...
									/>
									{/* Travelling light reflection across the glass. */}
									<span aria-hidden className="photo-sheen" />
								</div>
								...
								{placedBadges.map((badge, index) => {
									...
									return (
										<motion.span
											...
											className="neo-surface pointer-events-auto absolute inline-flex animate-float-soft items-center gap-1.5 rounded-full border border-line px-3 py-1.5 text-[11.5px] font-medium text-ink-muted backdrop-blur-xl transition-[transform,border-color,color,box-shadow] duration-300 ease-premium hover:scale-[1.12] hover:border-brand-500/60 hover:text-ink hover:shadow-[0_0_22px_rgba(59,130,246,0.35)]"
											style={{
												top: slot.top,
						...
						{/* Phone layout for the same badges. The floating anchors above
						    are hidden below `sm`, so on a phone the technologies would
						    disappear entirely — here they return as a swipeable rail that
						    fits the viewport instead of overflowing it. */}
						{placedBadges.length > 0 ? (
							<div className="chip-rail -mx-1 mt-5 flex snap-x snap-mandatory gap-2 overflow-x-auto px-1 pb-1 sm:hidden">
								{placedBadges.map((badge, index) => (
									<motion.span
										key={badge.id}
										...
										className="neo-surface inline-flex shrink-0 snap-start items-center gap-1.5 rounded-full border border-line px-3 py-1.5 text-[11.5px] font-medium text-ink-muted backdrop-blur-xl transition-transform duration-200 active:scale-95"
									>
										<Icon name={badge.icon} className="h-3.5 w-3.5" />
										{badge.label}
									</motion.span>
