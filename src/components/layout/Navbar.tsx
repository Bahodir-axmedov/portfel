	...
	return (
		<header
			className={cn(
				...
				scrolled
					? "border-b border-line bg-base/70 backdrop-blur-2xl"
					: "border-b border-transparent bg-transparent",
			)}
		>
			<nav
				className="container flex h-[76px] items-center justify-between gap-4"
				aria-label={t("menu")}
			>
				...
				<div className="flex items-center gap-2">
					...
					<button
						...
						aria-expanded={open}
						className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-line bg-white/[0.04] text-ink-muted transition-colors duration-200 hover:text-ink lg:hidden"
					>
						{open ? (
							...
							{NAV_ITEMS.map((item) => (
								<motion.li key={item.id} variants={rowVariants}>
									<a
										...
										className={cn(
											"flex items-center justify-between border-b border-white/[0.05] py-3.5 text-[15px] tracking-tight transition-colors duration-200",
