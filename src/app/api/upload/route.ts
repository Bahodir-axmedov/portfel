...
export async function POST(request: Request) {
	try {
		...
		// Logged only after the bytes are on disk, so the audit trail never
		// claims an upload that failed halfway through.
		void logActivity({
			action: "upload",
			resource: "media",
			entityId: filename,
			label: `${filename} (${Math.round(buffer.byteLength / 1024)} KB)`,
			actor: session.email,
