...
export async function POST(request: Request) {
	try {
		...
		// A restore overwrites live content, so it is the single most
		// destructive action in the panel. It must always leave a trace.
		void logActivity({
			action: "restore",
			resource: "backup",
			label: Object.keys(restored).join(", ") || null,
			actor: session.email,
